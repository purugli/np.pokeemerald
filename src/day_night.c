...
static const u16 sTimeOfDayTints[HOURS_PER_DAY][3] = {
    [0] = {TINT_NIGHT},
    [1] = {TINT_NIGHT},
    [2] = {TINT_NIGHT},
    [3] = {TINT_NIGHT},
    ...
    for (i = 0; i < ARRAY_COUNT(gPaletteOverrides); i++)
    {
        ...
        if (curr != NULL)
        {
            while (curr->slot != PALOVER_LIST_TERM && curr->palette != NULL)
            {
                ...
                if (IsBetweenHours(hour, curr->startHour, curr->endHour))
                {
...
{
    return LoadSpritePalette_HandleDNSTint(palette, TRUE);
}

void LoadCompressedPaletteWithDNSTint(const u32 *src, u32 offset, u32 size)
{
    LoadCompressedPalette_HandleDNSTint(src, offset, size, TRUE);
}

void LoadPaletteWithDNSTint(const void *src, u32 offset, u32 size)
{
    LoadPalette_HandleDNSTint(src, offset, size, TRUE);
}
...
void LoadCompressedPalette_HandleDNSTint(const u32 *src, u16 offset, u16 size, bool32 applyDNSTint)
{
    ...
    LoadPalette_HandleDNSTint(gPaletteDecompressionBuffer, offset, size, applyDNSTint);
}
