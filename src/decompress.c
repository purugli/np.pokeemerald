...
{
    u32 i;
    const struct CompressedSpriteSheet *monPicTable;

    i = GetIconSpecies(species, personality);
    if (!isFrontPic)
        monPicTable = gMonBackPicTable;
    else
        monPicTable = gMonFrontPicTable;

    LZ77UnCompWram(monPicTable[i].data, dest);
