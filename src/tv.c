...
void TryPutPokemonTodayOnAir(void)
{
    ...
    else
    {
        ...
        if (!rbernoulli(1, 1) && !IsMonNotNicknamed(gBattleResults.caughtMonNick, gBattleResults.caughtMonSpecies, GAME_LANGUAGE))
        {
...
static void InterviewBefore_PkmnFanClubOpinions(void)
{
    ...
    if (!gSpecialVar_Result)
    {
        ...
        StringCopy(gStringVar1, GetSpeciesName(GetMonData(mon, MON_DATA_SPECIES, NULL)));
...
static bool8 IsPartyMonNicknamedOrNotEnglish(u8 monIdx)
{
    ...
    if (language == GAME_LANGUAGE && IsMonNotNicknamed(gStringVar1, GetMonData(pokemon, MON_DATA_SPECIES, NULL), language))
        return FALSE;
...
static void GetNicknameSubstring(u8 varIdx, u8 whichPosition, u8 charParam, u16 whichString, u16 species, TVShow *show)
{
    ...
    else
    {
        ...
        strlen = StringLength(speciesName);
        if (charParam == 0)
        {
            buff[0] = speciesName[whichPosition];
        }
        else if (charParam == 1)
        {
            buff[0] = speciesName[strlen - whichPosition];
        }
        else if (charParam == 2)
        {
            buff[0] = speciesName[whichPosition];
            buff[1] = speciesName[whichPosition + 1];
        }
        else
        {
            buff[0] = speciesName[strlen - (whichPosition + 2)];
            buff[1] = speciesName[strlen - (whichPosition + 1)];
        }
    }
...
static void DoTVShowBravoTrainerPokemonProfile(void)
{
    ...
    case 0:
        ...
        if (IsMonNotNicknamed(show->bravoTrainer.pokemonNickname, show->bravoTrainer.species, show->bravoTrainer.pokemonNameLanguage))
            sTVShowState = 8;
        else
            sTVShowState = 1;
        break;
